 ...
 * управляемые часы — ни БД, ни сети. Портировано с gmail-mcp/scripts/
 * test-auto-execute.mjs (та же generic-логика consent.ts, только server/tool
 * имена свои).
 *
...
const cfg = { server: "docs", consentTtlMs: 3_600_000, minConsentGapMs: 5_000, sendBatchMax: 10, now };
...
const PAYLOAD = { account: "work", documents: [{ title: "Q4 план", initialText: "..." }] };
...
console.log("[1] happy path — манифест AWAITING_CONSENT, binding совпадает → исполняется");
{
  ...
  await store.createManifest({
    id: "m1", server: "docs", tool: "docs_create", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const result = await tryAutoExecute({ manifestId: "m1", tool: "docs_create", accountLabel: "work" }, rehashOk, store, cfg);
...
{
  const store = makeStore();
  const result = await tryAutoExecute({ manifestId: "nope", tool: "docs_create", accountLabel: "work" }, rehashOk, store, cfg);
...
console.log("\n[3] tool/accountLabel не совпадают с манифестом → null (защита от подмены)");
{
  const store = makeStore();
  await store.createManifest({
    id: "m3", server: "docs", tool: "docs_create", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const wrongTool = await tryAutoExecute({ manifestId: "m3", tool: "docs_append_text", accountLabel: "work" }, rehashOk, store, cfg);
  check("другой tool → null", wrongTool === null);
  const wrongAccount = await tryAutoExecute({ manifestId: "m3", tool: "docs_create", accountLabel: "personal" }, rehashOk, store, cfg);
...
console.log("\n[4] уже исполнен (гонка/повторный тик поллера) → null, повторно не исполняется");
{
  const store = makeStore();
  await store.createManifest({
    id: "m4", server: "docs", tool: "docs_create", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const first = await tryAutoExecute({ manifestId: "m4", tool: "docs_create", accountLabel: "work" }, rehashOk, store, cfg);
  check("первый вызов исполнился", first !== null);
  const second = await tryAutoExecute({ manifestId: "m4", tool: "docs_create", accountLabel: "work" }, rehashOk, store, cfg);
  check("второй вызов (та же гонка) → null, документ не создастся дважды", second === null);
...
console.log("\n[5] дрейф состояния (binding mismatch) → null, манифест НЕ инвалидируется молча");
{
  const store = makeStore();
  await store.createManifest({
    id: "m5", server: "docs", tool: "docs_create", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const result = await tryAutoExecute({ manifestId: "m5", tool: "docs_create", accountLabel: "work" }, rehashDrift, store, cfg);
...
console.log("\n[6] TTL истёк → null (тот же guard, что у обычного consumeManifest)");
{
  const store = makeStore();
  await store.createManifest({
    id: "m6", server: "docs", tool: "docs_create", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t - 4_000_000, expiresAt: clock.t - 1_000,
  });
  const result = await tryAutoExecute({ manifestId: "m6", tool: "docs_create", accountLabel: "work" }, rehashOk, store, cfg);
