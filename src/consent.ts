...
const CONSENT_MANNER_ALT = [...CONSENT_MANNER_WORDS]
  // Сортировка по УБЫВАНИЮ длины — как в Python-эталоне: длинные варианты
  // примеряются раньше своих префиксов («быстрее» до «быстро»).
  // ЧЕСТНО: здесь она ИЗБЫТОЧНА — правую границу `RB` внутри lookahead ниже
  // движок всё равно проверяет и откатывается на следующую альтернативу, что
  // мутационный тест и подтвердил (снятие ТОЛЬКО сортировки ничего не ломает,
  // снятие `RB` — ломает, см. scripts/test-consent-strict.mjs блок [3]).
  // Оставлена как вторая, дешёвая страховка и чтобы не расходиться с эталоном.
