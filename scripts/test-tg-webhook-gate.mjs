 ...
 *
 * Ported byte-for-byte in STRUCTURE from
 * gmail-mcp/scripts/test-tg-webhook-gate.mjs (mcp-development-standard
 * `references/development-pipeline.md` T2). Only the worker's fake `Config`
 * object differs: docs-mcp's `Config` (src/config.ts) has no
 * `sendingStuckMinutes` field (that's gmail-mcp-specific, for its snooze
 * feature), so it's simply omitted here.
 *
 * Context: `consumeTgDecisionAnyServer` made webhook *consume*
 ...
 * designated webhook owner (`TG_WEBHOOK_OWNER` unset/false — which is the
 * PERMANENT state for docs-mcp, per this port's task notes: docs-mcp must
 * never own the webhook) must refuse the route entirely, even with the
 * CORRECT secret, before the handler runs.
 *
 ...
 * `test-s5-failclosed.mjs`-style tests use for MCP_AUTH_TOKEN scenarios):
 * unlike `requireAuth`, `TG_WEBHOOK_OWNER` is NOT threaded through the
 * `Config` object passed to `startHttpServer` -- the route handler closes
 * over the module-level singleton `tgApprovalConfig` (`src/server.ts`),
 * computed ONCE from `process.env` at import time. A single process can only
 * ever observe one value of it. So each scenario below gets its own `node`
 * process with its own env, self-invoking this same file with
 * `TG_TEST_WORKER` set (worker mode), and the parent (orchestrator mode, no
 * env var set) collects + checks the JSON result line each worker prints.
 *
 ...
 * `src/index.ts`'s real startup, which this test bypasses), so `store.ts`'s
 * internal pg Pool is never constructed. If `handleWebhook` ->
 * `consumeTgDecisionAnyServer` is ever actually invoked, it throws "Store not
 * initialised", caught by http.ts's `try { await handleWebhook(...) } catch
 * (err) { console.error("TG approval webhook error:", err); }` -- a
 * distinctive, capturable log line that only exists if execution reached
 * past the gate into the real handler. Absence of that line (owner=false) is
 * the "handler not called" proof; presence of it exactly once (owner=true) is
 * the "handler WAS called, same as before this fix" regression proof.
 *
...
function runOrchestrator() {
  ...
  // Existence-only control to prove the gate itself isn't broken -- docs-mcp
  // must NEVER actually set TG_WEBHOOK_OWNER=true in production (see this
  // port's task notes: docs-mcp never owns the webhook).
