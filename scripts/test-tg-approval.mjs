 ...
 * что `store.ts`.
 *
 * Ported byte-for-byte in STRUCTURE from gmail-mcp/scripts/test-tg-approval.mjs
 * (mcp-development-standard `references/development-pipeline.md` T2 — same
...
const consentCfg = { server: "docs", consentTtlMs: 3_600_000, minConsentGapMs: 5_000, sendBatchMax: 10, now };

const PAYLOAD = { account: "work", documents: [{ title: "Q3 Plan", initialText: "..." }] };
const OBJHASH = sha256(PAYLOAD);
const plan = () => ({
  payload: PAYLOAD,
  objectHash: OBJHASH,
  preview: "### 📤 План: Создание документов — 1\n\n- **«Q3 Plan»**",
  batchSize: 1,
});
...
console.log("\n[1] TG_APPROVAL выключен → requireConsent ведёт себя как без tg вовсе");
{
  ...
  check("enabledFor() всегда false, когда enabled=false", !gate.enabledFor("docs_create"));

  const storeA = makeConsentStore();
  const decA = await requireConsent({ tool: "docs_create", accountLabel: "work", plan, rehash, store: storeA, cfg: consentCfg });

  const storeB = makeConsentStore();
  const decB = await requireConsent({ tool: "docs_create", accountLabel: "work", plan, rehash, store: storeB, cfg: consentCfg, tg: gate });
  ...
  const decAExec = await requireConsent({
    tool: "docs_create", accountLabel: "work", manifestId: decA.manifestId, userReply: "да",
    plan, rehash, store: storeA, cfg: consentCfg,
  });
  const decBExec = await requireConsent({
    tool: "docs_create", accountLabel: "work", manifestId: decB.manifestId, userReply: "да",
...
let planCtx; // переиспользуем в [4]/[5]/[6]/[7]
{
  ...
  // (see the file's top note), `row` is undefined here — a plain
  // `row.messageId` would crash the whole script instead of reporting FAIL.
  check("message_id из ответа Telegram сохранён", !!row && row.messageId === 4242);
...
console.log("\n[4] execute при PENDING в Telegram → refused «жду подтверждения», манифест ЖИВ");
{
  ...
  const exec = await requireConsent({
    tool: "docs_create", accountLabel: "work", manifestId: dec.manifestId, userReply: "да",
...
console.log("\n[7b] кнопка нажата после истечения approval-TTL (манифест ещё жив) → решение НЕ записывается");
{
  ...
  // Guarded (unlike gmail-mcp's original): without consent.ts's `tg` wiring
  // (see the file's top note), `rowAfter` is undefined here — a plain
  // `rowAfter.status` would crash the whole script instead of reporting FAIL.
  ...
  check(
    ...
    !!rowAfter && rowAfter.status === "PENDING",
    JSON.stringify(rowAfter),
  );
  check("decidedAt не проставлен", !rowAfter || rowAfter.decidedAt === null);
  check(
...
console.log("\n[9] webhook: callback от НЕ владельца → игнорируется, approval не меняется");
{
  ...
  const cfg = tgCfg();
  const tgStore = makeTgStore();
  await tgStore.createTgApproval({ manifestId: "m-owner-check", server: "docs", chatId: cfg.ownerChatId, messageId: 1, createdAt: now(), expiresAt: now() + 3_600_000 });
...
console.log("\n[10] webhook: повторный (replay) callback того же решения — второй раз no-op");
{
  ...
  const cfg = tgCfg();
  const tgStore = makeTgStore();
  await tgStore.createTgApproval({ manifestId: "m-replay", server: "docs", chatId: cfg.ownerChatId, messageId: 2, createdAt: now(), expiresAt: now() + 3_600_000 });
...
// Регрессионный тест на сам фикс: cfg этого процесса — "docs" (webhook может
// быть физически задеплоен на любом из 6 серверов; здесь этот процесс), а
// approval-строка в БД принадлежит "calendar" (создана ДРУГИМ сервером через
// тот же общий бот-токен). Со старым фильтром `AND server = cfg.server` в
...
// сервера совпали.
console.log("\n[12] вебхук на docs консюмит approval манифеста, принадлежащего server=calendar");
{
  const { mock } = resetTelegramMocks();
  mock("answerCallbackQuery", () => ({ statusCode: 200, data: { ok: true }, headers: { "content-type": "application/json" } }));
  mock("editMessageReplyMarkup", () => ({ statusCode: 200, data: { ok: true }, headers: { "content-type": "application/json" } }));

  // cfg.server = "docs" (этот процесс), но approval-строка принадлежит "calendar".
  const cfg = tgCfg({ server: "docs" });
  const tgStore = makeTgStore();
  ...
  // с cfg.server="docs" против строки server="calendar" — 0 rows, silent miss.
  ...
  check(
    "новый server-agnostic путь: вебхук на docs консюмит APPROVED для манифеста server=calendar",
    row.status === "APPROVED",
    JSON.stringify(row),
  );
  ...
  // REJECTED-ветка тем же способом, другой манифест чужого сервера ("gmail").
  await tgStore.createTgApproval({
  ...
  check(
    "REJECTED тоже консюмится через чужой сервер (gmail)",
...
// TG_APPROVAL_ENABLED. docs-mcp В ЧАСТНОСТИ никогда не должен владеть
// вебхуком (TG_WEBHOOK_OWNER=true запрещён этим порту в задании) — этот тест
// проверяет тот же guard, на который это опирается.
console.log("\n[13] registerWebhook: TG_WEBHOOK_OWNER не установлен/false → setWebhook НЕ вызывается");
{
  ...
  // (Это не значит, что docs-mcp сам когда-либо ставит TG_WEBHOOK_OWNER=true в
  // проде — только что функция registerWebhook как таковая не сломана.)
